# Runs run_release.ps1 to set envs and start the app, then kills it after a timeout.
param(
  [int]$Seconds = 180
)
$ErrorActionPreference = 'Stop'
# Launch run_release.ps1 in its own PowerShell so its working directory changes don't affect us
...
$runScript = Join-Path $repoRoot 'run_release.ps1'
if (!(Test-Path $runScript)) { throw "run_release.ps1 not found at $runScript" }
