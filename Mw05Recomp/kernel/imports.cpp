...
uint32_t NtWaitForSingleObjectEx(uint32_t Handle, uint32_t WaitMode, uint32_t Alertable, be<int64_t>* Timeout)
{
    ...
    else
    {
        ...
        if (Handle != 0 && Handle < PPC_MEMORY_SIZE)
        {
...
{
    InitVmArenaOnce();
    // Validate guest pointers: ensure they lie within the emulated memory window.
    ...
    // Align size to 64 KiB region granularity; many callers depend on this.
    ...
    if (is_reserve || (!is_commit))
    {
        ...
        if (reserved == 0)
        {
            const char* t = std::getenv("MW05_TRACE_NTALLOC"); const char* m = std::getenv("MW05_TRACE_MEM");
            const bool on = (t && !(t[0]=='0' && t[1]=='\0')) || (m && !(m[0]=='0' && m[1]=='\0'));
            if (on) LOGFN("[ntalloc] reserve failed hint=0x{:08X} size={}", in_base, aligned_size);
    ...
    if (is_commit)
    {
        // If commit follow-up, prefer honoring the hinted base if it matches a reservation.
        const uint32_t commit_base = (in_base != 0) ? in_base : static_cast<uint32_t>(*BaseAddress);
        bool satisfied = false;
        {
            std::lock_guard<std::mutex> lk(g_NtAllocMutex);
            auto it = g_NtReservations.find(commit_base);
            if (it != g_NtReservations.end())
            {
                // Constrain commit size; our backing is already reserved+zeroed.
                const uint32_t commit_size = align_up(static_cast<uint32_t>(*RegionSize), page4k);
                if (commit_size <= it->second.Size)
                {
                    out_guest = it->second.GuestBase;
                    *BaseAddress = out_guest;
                    *RegionSize  = commit_size;
                    satisfied = true;
                }
            }
        }
        ...
        if (!satisfied)
        {
            // Unknown commit (no reservation). Fall back to a fresh aligned region.
            const uint32_t alloc_size = aligned_size + (region64k * 2);
            {
                const char* t1 = std::getenv("MW05_TRACE_NTALLOC");
                const char* t2 = std::getenv("MW05_TRACE_MEM");
                const bool on = (t1 && !(t1#include <stdafx.h>
#include <cpu/ppc_context.h>
#include <cpu/guest_thread.h>
#include <apu/audio.h>
#include "function.h"
...
// ExCreateThread signature (Xbox 360):
// r3: be<uint32_t>* ThreadHandle (out)
// r4: uint32_t StackSize (0 = default)
// r5: be<uint32_t>* ThreadId (out, optional)
// r6: uint32_t XapiThreadStartup (ignored)
// r7: uint32_t StartRoutine (guest address)
// r8: uint32_t StartContext (guest value passed in r3)
// r9: uint32_t CreationFlags (bit 0 = start suspended)
uint32_t ExCreateThread(
    be<uint32_t>* outHandle,
    uint32_t stackSize,
    be<uint32_t>* outThreadId,
    uint32_t /*xapiThreadStartup*/,
    ...
    uint32_t creationFlags)
{
    ...
    if (!valid_ptr(outHandle, sizeof(*outHandle)))
        return 0xC000000DL; // STATUS_INVALID_PARAMETER

    // Create guest thread handle
    uint32_t tid = 0;
    GuestThreadHandle* h = GuestThread::Start({ startRoutine, startContext, creationFlags }, &tid);
    const uint32_t handle = GetKernelHandle(h);

    // Write outs (big-endian wrappers)
    *outHandle = handle;
    if (outThreadId && valid_ptr(outThreadId, sizeof(*outThreadId)))
        *outThreadId = tid;

    (void)stackSize; // default stack size used inside GuestThreadHandle
    return 0; // STATUS_SUCCESS
}
    ...
    uint32_t Protect)
{
    ...
    if (is_reserve || (!is_commit))
    {
        ...
        {
            const char* t = std::getenv("MW05_TRACE_NTALLOC"); const char* m = std::getenv("MW05_TRACE_MEM");
            const bool on = (t && !(t[0]=='0' && t[1]==' ')) || (m && !(m[0]=='0' && m[1]==' '));
            if (on) LOGFN("[ntalloc] reserve failed hint=0x{:08X} size={}", in_base, aligned_size);
            return 0xC0000018; // STATUS_CONFLICTING_ADDRESSES
        }
        out_guest = reserved;
    ...
    if (is_commit)
    {
        ...
        if (!satisfied)
        {
            ...
            const uint32_t alloc_size = aligned_size + (region64k * 2);
            {
                ...
                if (on)
                {
                    const auto d = o1heapGetDiagnostics(g_userHeap.heap);
                    bool ok = o1heapDoInvariantsHold(g_userHeap.heap);
                    LOGFN("[ntalloc.pre] commit-fallback req={} aligned={} alloc_size={} heap_alloc={}/{} oom={} invariants={}",
                          size, aligned_size, alloc_size, d.allocated, d.capacity, (unsigned long long)d.oom_count, ok?"ok":"bad");
                }
            }
            void* host_original = g_userHeap.Alloc(alloc_size);
            if (!host_original)
                return 0xC0000017; // STATUS_NO_MEMORY

            uintptr_t p = reinterpret_cast<uintptr_t>(host_original);
            uintptr_t aligned = (p + (region64k - 1)) & ~(static_cast<uintptr_t>(region64k - 1));
            if (aligned - p < 16) aligned += region64k;
            void* host_aligned = reinterpret_cast<void*>(aligned);
            *((uint64_t*)host_aligned - 2) = Heap::kAlignedMagic;
            *((void**)host_aligned - 1) = host_original;
            memset(host_aligned, 0, aligned_size);
            out_guest = g_memory.MapVirtual(host_aligned);

            {
                std::lock_guard<std::mutex> lk(g_NtAllocMutex);
                g_NtReservations[out_guest] = NtReservation{ out_guest, aligned_size, host_original, host_aligned };
            }
    ...
    // New commit path using VmArena
    if (is_commit)
    {
        uint32_t commit_base = (out_guest != 0) ? out_guest : (in_base != 0 ? in_base : static_cast<uint32_t>(*BaseAddress));
        if (commit_base == 0)
        {
            // No hint and nothing reserved yet: reserve now.
        ...
        if (!g_vmArena.Commit(commit_base, aligned_size))
        {
            const char* t = std::getenv("MW05_TRACE_NTALLOC"); const char* m = std::getenv("MW05_TRACE_MEM");
            const bool on = (t && !(t[0]=='0' && t[1]=='\0')) || (m && !(m[0]=='0' && m[1]=='\0'));
            if (on) LOGFN("[ntalloc] commit failed base=0x{:08X} size={}", commit_base, aligned_size);
            return 0xC0000018; // conflict/not allocated
        }

        void* host = g_memory.Translate(commit_base);
        memset(host, 0, aligned_size);
    ...
    // Optional debug trace
    {
        ...
        if (on)
        {
            ...
            LOGFN("[ntalloc] base_ptr=0x{:08X} size_ptr=0x{:08X} in_base=0x{:08X} alloc_type=0x{:08X} protect=0x{:08X} req={} aligned={} out=0x{:08X}",
                  base_ptr, size_ptr, in_base, AllocationType, Protect, (uint32_t)*RegionSize, aligned_size, static_cast<uint32_t>(*BaseAddress));
        }
    }
...
uint32_t KeWaitForSingleObject(XDISPATCHER_HEADER* Object, uint32_t WaitReason, uint32_t WaitMode, bool Alertable, be<int64_t>* Timeout)
{
    ...
    if (const char* t = std::getenv("MW05_TRACE_KERNEL"); t && t[0] && !(t[0]=='0' && t[1]==0))
    {
        LOGFN("[ke.wait] obj=0x{:08X} type={} state={} alertable={} timeout={}", g_memory.MapVirtual(Object), (unsigned)Object->Type, (unsigned)Object->SignalState, Alertable?1:0, timeout);
    }
...
// duplicate ExCreateThread hook removed (defined earlier)
